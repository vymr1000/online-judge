...
public class RenewMenu {
    ...
    Map<String, Integer> menuCombList = new HashMap<>();
    // 베스트 메뉴를 담을 List 선언
    ...
    public List<String> solution(String[] orders, int[] course) {
        ...
        for (int k : course) {
            ...
            for (String order : orders) {
                ...
                combination(arr, new boolean[arr.length], 0, 0, k);
                // HashMap에서 Value로 정렬하기 위한 key 리스트 선언
                List<String> keySet = new ArrayList<>(menuCombList.keySet());
                // Value로 정렬
                keySet.sort((o1, o2) -> (menuCombList.get(o2).compareTo(menuCombList.get(o1))));
            }
